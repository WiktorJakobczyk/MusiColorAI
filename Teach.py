...
def createSequenceExamplesPoly(config,input,output_dir,eval_ratio):
    os.system('python magenta/magenta/models/polyphony_rnn/polyphony_rnn_create_dataset.py \
    --config='+config+' \
    --input='+input+' \
    --output_dir='+output_dir+' \
    --eval_ratio='+str(eval_ratio))
...
def trainPoly(config,run_dir,sequence_example_file,batch_size,rnn_layer_sizes,num_training_steps):
    ...
    --run_dir='+run_dir+' \
    --sequence_example_file='+sequence_example_file+' \
    --hparams="batch_size='+str(batch_size)+',rnn_layer_sizes=['+str(rnn_layer_sizes[0])+','+str(rnn_layer_sizes[1])+','+str(rnn_layer_sizes[2])+']" \
    --num_training_steps='+str(num_training_steps))
...
def evaluatePoly(config,run_dir,output_dir,num_outputs,num_steps,batch_size,rnn_layer_sizes,primer_melody):
